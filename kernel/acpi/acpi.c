...
static struct acpi_sdt_header *rsdt_find(struct acpi_rsdt *rsdt, const char *signature) {
	...
	for (int i = 0; i < entries; i++) {
		struct acpi_sdt_header *hdr = (struct acpi_sdt_header *) pmm_to_virt(rsdt->sdts[i]);
		if (memcmp((void *) hdr->signature, (void *) signature, 4) == 0) {
			header = hdr;
			break;
		}
	}
...
__unused static void print_sdt_entries(struct acpi_rsdt *rsdt) {
	int entries = (rsdt->header.len - sizeof(rsdt->header)) / 4;
	for (int i = 0; i < entries; i++) {
		struct acpi_sdt_header *hdr = (struct acpi_sdt_header *) (u64) rsdt->sdts[i];
		kprintf("%.4s\n", hdr->signature);
	}
}
